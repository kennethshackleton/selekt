    ...
    fun `vec1 computes l2 squared distance on real database`() {
        ...
        try {
            ...
            try {
                ...
                assertTrue(kotlin.math.abs(distance - 25.0) < 1e-9, "Expected squared distance 25.0, got $distance")
            } finally {
                sqlite.finalize(statement)
            }
        } finally {
    ...
    fun `vec1 vector json round trip works on real database`() {
        ...
        try {
            ...
            try {
                ...
                assertTrue(kotlin.math.abs(element - 2.0) < 1e-9, "Expected element 2.0, got $element")
            } finally {
                sqlite.finalize(statement)
            }
        } finally {
        ...
        try {
            ...
            try {
                sqlite.run {
                    ...
                    assertTrue(kotlin.math.abs(columnDouble(statement, 1) - 0.0) < 1e-9)
                    assertEquals(SQL_ROW, step(statement))
                    assertEquals(2L, columnInt64(statement, 0))
                    assertTrue(kotlin.math.abs(columnDouble(statement, 1) - 1.0) < 1e-9)
                    assertEquals(SQL_ROW, step(statement))
                    assertEquals(3L, columnInt64(statement, 0))
                    assertTrue(kotlin.math.abs(columnDouble(statement, 1) - 4.0) < 1e-9)
                }
            } finally {
                sqlite.finalize(statement)
            }
